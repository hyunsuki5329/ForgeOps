...
이 문서는 ForgeOps가 해결할 제품 문제와 Phase 0~4에서 충족해야 할 기능·비기능 요구사항, 성공 지표, 릴리스 수준 Exit gate를 정의한다. PRD는 제품이 **무엇을** 제공하고 **왜** 필요한지의 기준 소유 문서다. 구현 작업, 의존성, 일정, 완료 정의는 `docs/project/wbs.md`가 소유하며 이 문서에서 구현 순서나 상세 일정을 정하지 않는다.
...
ForgeOps는 자연어 개발 요청을 구조화된 작업 계약으로 바꾸고, 고정 snapshot과 격리 환경에서 계획·실행·검증·복구한 뒤 근거와 trace를 사람에게 제공하는 자체 평가형 AI 개발 워크플로 플랫폼이다.
...
- Issue에서 Patch를 거쳐 Draft PR로 이어지는 흐름
- 의존성/API 마이그레이션
- CI 실패 복구
- 대상이 지정된 리팩터링
- 보안/품질 개선
...
현재 위치는 **Harness Foundation과 제품 Phase 0 W1 bridge 완료**다. 현재 저장소에서 확인 가능한 구현과 향후 제품 runtime을 다음처럼 구분한다.
...
| Product Task Contract bridge | IMPLEMENTED | versioned schema와 lossless/non-grant fixture가 fresh VG-002 E2를 통과 |
...
| ID | Phase | 우선순위 | 성숙도 | 요구사항 | 수용 결과 | 검증 |
| --- | --- | --- | --- | --- | --- | --- |
| PRD-FR-001 | Phase 0 | 필수 | IMPLEMENTED | Product Task Contract를 TaskPacket으로 lossless 정규화하며 Contract가 authority를 만들지 않는다.<br>근거: 자연어 해석 손실과 계약을 통한 암묵적 권한 확대를 차단해야 한다. | 원문, criterion, constraint, source identity가 매핑 후 보존되고 authority는 runtime 관찰과 Main 결정 밖에서 추가되지 않는다. | VG-002 |
...
아래 gate는 릴리스 수준 수용 결과다. 현재 제품 runtime이 없으므로 Phase 0~4의 현재 결과는 모두 `NOT_RUN`이며, gate 문구는 현재 달성 주장이나 일정 약속이 아니다.
...
| 지표 | 계산과 보고 규칙 |
| --- | --- |
| 작업 성공률 | 모든 필수 criterion이 요구 evidence floor에서 `PASSED`인 scheduled run / 전체 scheduled run |
| 회귀율 | baseline에서 통과했으나 변경 후 실패한 check / baseline 통과 check. 분모가 0인 case는 제외하지 않고 `BASELINE_UNHEALTHY`로 별도 보고 |
| 복구 성공률 | `RECOVERING` 진입 후 budget 안에 `SUCCEEDED`한 run / `RECOVERING` 진입 run |
| 진전 없음 중단 | 반복 failure signature와 evidence/diff 변화 없음이라는 정답 label에 대한 precision과 recall |
| 무권한 작업률 | 유효한 exact authority 없이 실제 실행된 action / 전체 action attempt. 목표는 0 |
| 주입 탈출률 | 비신뢰 명령 승격, secret 노출 또는 무권한 효과가 발생한 공격 case / 전체 공격 case |
| 무관한 변경률 | 허용 범위 밖 changed line / 전체 changed line. generated file은 별도 보고 |
| 사람에 의한 되돌림률 | 승인 후 14일 관찰을 완료한 변경 중 결함 때문에 revert된 변경 / 관찰 완료 변경 |
| 지연 시간 | task accepted부터 terminal state까지 p50/p95. 사람 승인 대기 시간은 별도 분리 |
| 비용 | scheduled run당 token/API 비용과 성공 run당 비용, tool call 수 |

Phase 2의 핵심 연구 질문은 실행 기반 검증, bounded recovery, trace-level evaluation을 결합한 ForgeOps가 단순 tool-using agent보다 작업 성공률을 높이고 회귀율과 불필요한 변경을 낮추는지다. 비교 delta는 `ForgeOps Full - Tool-using agent without baseline and recovery`로 정의하며 다음 조건을 모두 만족해야 개선을 주장할 수 있다.
...
아래 값은 모두 `PLANNED` 제품 runtime의 릴리스 목표이며 현재 측정치가 아니다.

| SLI | 측정 | PLANNED 목표 | 현재 측정 |
| --- | --- | --- | --- |
| 이벤트 전파 | durable event commit부터 UI 수신까지 | rolling 30일 p99 ≤ 2초 | 없음 |
| 샌드박스 정리 | terminal/cancel 수락부터 process·mount·lease가 0이 될 때까지 | rolling 30일 p99 ≤ 60초, hard max 5분 | 없음 |
| 추적 완전성 | 필수 phase span과 artifact reference가 완전한 terminal run 비율 | rolling 30일 ≥ 99.5% | 없음 |
| 매니페스트 완전성 | schema valid하고 모든 reference가 해석되는 terminal run 비율 | rolling 30일 ≥ 99.9% | 없음 |
| R2 재현성 | 사전 고정한 replay-eligible 표본 중 deterministic check 결과가 동일한 비율 | release별 ≥ 95% | 없음 |
| API 가용성 | 유효 요청 중 non-5xx 응답 비율 | rolling 30일 ≥ 99.9% | 없음 |
