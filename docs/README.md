...
현재 제품 runtime, sandbox, durable persistence, 외부 publisher와 제품
검증 결과는 구현 또는 통과된 것으로 간주하지 않는다. 문서에 정의된
...
2. [제품 요구사항 문서](product/prd.md) — Phase 0~4의 목표, 범위, 요구사항과 release-level 수용 결과를 읽는다.
3. [시스템 아키텍처](architecture/system-architecture.md) — 제품과 Protocol 경계, 구성요소, 상태·계약·흐름을 확인한다.
4. [위협 모델](security/threat-model.md) — 보호 자산, 위협, 보안 통제와 phase-blocking gate를 확인한다.
5. [검증 및 평가 계획](quality/verification-and-evaluation-plan.md) — evidence 계약, VG, 평가와 Phase Exit 판정 방법을 읽는다.
6. [작업 분해 구조](project/wbs.md) — Phase 0~1의 W1~W10 상세 계획과 Phase 2~4 상위 마일스톤을 확인한다.
7. [위험 등록부](project/risk-register.md) — 일정·제품·기술·보안·평가·포트폴리오 위험과 대응 시점을 검토한다.
8. [요구사항 추적성 매트릭스](project/requirements-traceability-matrix.md) — 요구사항에서 설계·통제·작업·검증까지의 planned linkage와 공백을 확인한다.
