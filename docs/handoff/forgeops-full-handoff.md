...
> **ForgeOps는 저장소 변경을 계획해 격리된 sandbox에서 실행하고, 결정론적 검사로 결과를 검증하며, 실패를 복구하고, 사람이 검토할 수 있도록 end-to-end trace를 제공하는 검증 가능한 AI development workflow platform이다.**
...
| 항목 | 질문 |
|---|---|
| 목표 달성 | Task Contract 목표를 실제로 충족했는가 |
| 계획 품질 | 제약, 위험, 검증 조건을 반영했는가 |
| 근거 기반성 | 실제 코드, 로그, 문서 근거에 기반했는가 |
| 도구 선택 | 적절한 도구를 올바른 순서로 썼는가 |
| 복구 품질 | 실패를 정확히 분류하고 진전 있는 수리를 했는가 |
| patch 최소성 | 불필요한 리팩터링과 범위 확장을 피했는가 |
| 안전성 | 정책과 승인 경계를 지켰는가 |
| 검토 용이성 | 사람이 diff와 근거를 이해할 수 있는가 |
...
9. **Policy and Audit Console**
   도구 허용 목록, 승인 매트릭스, MCP 신뢰 목록, 통합 범위, 감사 내보내기.
...
| SLI | 측정 | 목표와 window |
|---|---|---|
| 이벤트 전파 | durable event commit부터 UI 수신까지 | rolling 30일 p99 ≤ 2초 |
| sandbox 정리 | terminal/cancel 수락부터 process·mount·lease가 0이 될 때까지 | rolling 30일 p99 ≤ 60초, hard max 5분 |
| trace 완전성 | 필수 phase span과 artifact reference가 완전한 terminal run 비율 | rolling 30일 ≥ 99.5% |
| manifest 완전성 | schema valid하고 모든 reference가 해석되는 terminal run 비율 | rolling 30일 ≥ 99.9% |
| R2 재현성 | replay 지원 표본 중 deterministic check 결과가 동일한 비율 | release별 ≥ 95% |
| API 가용성 | 유효 요청 중 non-5xx 응답 비율 | rolling 30일 ≥ 99.9% |
...
| 작업 | 선호 특성 |
|---|---|
| task 파싱, 로그 분류 | 빠른 structured output, 저비용 |
| 계획, 진단 | 높은 reasoning 품질 |
| 코드 수정 제안 | code quality와 tool use |
| 최종 설명 | 읽기 쉬운 요약 |
| rubric 평가자 | 독립 evaluator 또는 ensemble |
